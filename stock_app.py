import streamlit as st
import pandas as pd
import yfinance as yf
import plotly.graph_objects as go
import requests
import urllib3

# 1. 基礎設定：忽略 SSL 警告 (修復證交所 API 錯誤)
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
st.set_page_config(page_title="100萬實戰-終極不卡版", layout="wide", initial_sidebar_state="expanded")

# 初始化帳戶
for key, val in {'balance': 1000000.0, 'position': 0, 'buy_price': 0.0}.items():
    if key not in st.session_state: st.session_state[key] = val

# --- 【左側指揮中心】 ---
with st.sidebar:
    st.header("🕹️ 指揮中心")
    st.metric("💰 剩餘現金", f"${st.session_state.balance:,.0f}")
    target_code = st.text_input("🔍 代碼演練 (如: 2330)", value="2330").strip()
    st.divider()
    if st.button("♻️ 徹底刷新頁面"): st.rerun()

# --- 【右側主畫面】 ---
st.title("🛡️ 100萬實戰：全功能戰情室")

symbol = f"{target_code}.TW" if target_code != "^TWII" else "^TWII"

# 2. 數據抓取模組 (加入強大錯誤處理)
with st.spinner('連線衛星數據中...'):
    try:
        # 抓取 5 天內的 5 分鐘 K 線 (多抓幾天確保有資料)
        df = yf.download(symbol, period="5d", interval="5m", timeout=15, progress=False)
        
        if not df.empty:
            # 取得最新一筆價格
            cur_p = float(df['Close'].iloc[-1].iloc[0] if isinstance(df['Close'].iloc[-1], pd.Series) else df['Close'].iloc[-1])
            
            # --- 情報看板 ---
            st.subheader(f"📊 {target_code} 關鍵財務看板")
            c1, c2, c3, c4 = st.columns(4)
            
            # 抓取基本面 (不影響主畫面)
            try:
                t = yf.Ticker(symbol)
                info = t.info
                c1.metric("當前股價", f"{cur_p:.2f}")
                c2.metric("🧬 EPS", f"{info.get('trailingEps', 'N/A')}")
                c3.metric("💰 毛利率", f"{info.get('grossMargins', 0)*100:.1f}%")
                c4.metric("📈 ROE", f"{info.get('returnOnEquity', 0)*100:.1f}%")
            except:
                c1.metric("當前股價", f"{cur_p:.2f}")
                st.caption("⚠️ 財報數據暫時無法連線，僅顯示行情。")

            # --- 交易執行 ---
            st.divider()
            t1, t2, t3 = st.columns([1, 1, 2])
            with t1:
                if st.button("🔴 買進 1 張", use_container_width=True):
                    st.session_state.balance -= cur_p * 1000
                    st.session_state.position += 1
                    st.session_state.buy_price = cur_p
                    st.rerun()
            with t2:
                if st.button("🟢 全數平倉", use_container_width=True):
                    if st.session_state.position > 0:
                        st.session_state.balance += (cur_p * st.session_state.position * 1000)
                        st.session_state.position = 0; st.session_state.buy_price = 0.0
                        st.rerun()
            with t3:
                p_l = (cur_p - st.session_state.buy_price) * st.session_state.position * 1000 if st.session_state.position > 0 else 0
                st.metric("持倉損益", f"{p_l:,.0f}", delta=f"{(p_l/1000000)*100:.2f}%" if st.session_state.position > 0 else None)

            # --- K 線圖繪製 (修復座標軸) ---
            fig = go.Figure(data=[go.Candlestick(
                x=df.index, open=df['Open'], high=df['High'],
                low=df['Low'], close=df['Close'], name='K線'
            )])
            if st.session_state.position > 0:
                fig.add_hline(y=st.session_state.buy_price, line_dash="dash", line_color="red")
            
            fig.update_layout(
                height=500, template="plotly_dark", 
                xaxis_rangeslider_visible=False,
                margin=dict(l=10, r=10, t=10, b=10),
                yaxis_title="價格 (TWD)"
            )
            st.plotly_chart(fig, use_container_width=True)
            
        else:
            st.error("❌ 無法取得報價數據。可能原因：非交易時段或代碼輸入有誤。")
            
    except Exception as e:
        st.warning(f"📡 數據中轉站繁忙中... 請輸入代碼後多按幾次 Enter。")
